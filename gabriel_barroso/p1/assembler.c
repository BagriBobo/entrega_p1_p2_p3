...
void write_mem_dump(const char *filename) {
...
int main(int argc, char *argv[]) {
    ...
    // Gera arquivo .mem, exceto se o nome já termina com .mem
    const char *out = argv[2];
    size_t len = strlen(out);
    int gerou_mem = 0;
    char memname[512];

    if (len >= 4 && strcmp(out + len - 4, ".mem") == 0) {
        // Só gera o .mem
        write_mem_dump(out);
        printf("Arquivo binário %s gerado com sucesso!\n", out);
    } else {
        // Gera .bin e .mem
        strncpy(memname, out, sizeof(memname));
        memname[sizeof(memname)-1] = '\0';
        char *dot = strrchr(memname, '.');
        if (dot) strcpy(dot, ".mem");
        else strcat(memname, ".mem");
        write_mem_dump(memname);
        printf("Arquivos binários %s e %s gerados com sucesso!\n", out, memname);
    }
